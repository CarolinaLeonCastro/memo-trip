	...
	static async searchPlaces(query, limit = 5, countryCode = null) {
		try {
			...
			const response = await fetch(`${NOMINATIM_BASE_URL}/search?${params}`, {
				headers: {
					'User-Agent': 'MemoTrip/1.0 (your-email@example.com)' // Remplacez par votre email
				}
			});
			...
			logger.error('Geocoding search error', { error: error.message, query });
	...
	static async reverseGeocode(latitude, longitude) {
		try {
			...
			const response = await fetch(`${NOMINATIM_BASE_URL}/reverse?${params}`, {
				headers: {
					'User-Agent': 'MemoTrip/1.0 (your-email@example.com)'
				}
			});
